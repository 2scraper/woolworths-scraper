...
scrapers, regardless of what URL was requested (search, category, anything) —
this is deliberate, not scoped to any one page. A challenge that nobody
recognises becomes an empty category months later (CLAUDE.md §8).

WHERE THIS STANDS ON WOOLWORTHS
--------------------------------
Nothing here has been observed firing on woolworths.com.au, and that is
measured rather than assumed: counted across every capture taken for this
repo, `sitekey`, `data-sitekey`, `recaptcha`, `hcaptcha` and `turnstile` are
each **0** on every served page and on both Akamai denial pages. Akamai's
refusal is about 400 bytes of plain HTML — a title, an `<h1>`, one sentence
and a reference number — with no widget of any kind on it. So no solve is
attempted on this site and nothing is charged.

Say that precisely, because the sentence next door is the most expensive
mistake this family has made (§19): **that is a statement about what these
pages carry, not about what a solver can do.** "Unsolvable" is a property of
a page with no widget on it. The capabilities below are real and implemented,
and if Woolworths ever serves an interstitial that carries a widget,
`page_flow.STATE_POLICY`'s `solve` flag is the one line to change.

ONE CAVEAT SPECIFIC TO DEBUGGING THIS SITE: a page fetched over
`--cdp-endpoint` is NOT clean. The 2Captcha Scraping Browser's own auto-solve
extension injects its hunters into every page it loads, so a served
Woolworths listing that counts 0 captcha markers locally counts `turnstile`
3, `recaptcha` 2 and `<captcha` 1 when fetched that way. Those are the
extension's, not the site's. The detectors below deliberately key on
Cloudflare's own furniture and on a class-plus-sitekey pattern the
extension's script tag cannot match, and `smoke_test.py` pins that — because
the failure mode is paying for a solve of a captcha that was never there.

WHAT IT IMPLEMENTS, AND WHERE THAT WAS PROVEN
----------------------------------------------
The measurements below were taken on a SIBLING repo (foodpanda-scraper),
which is where this module's capabilities were built and paid for. They are
kept verbatim rather than trimmed, because they are the evidence that these
task types work — and because a claim about a vendor's capability that cites
no task type is a guess wearing a fact's clothes.

**Enterprise reCAPTCHA.** PerimeterX's denial page there carries a
`g-recaptcha` container that looks like an ordinary v2 checkbox, and the
loader beside it is the ENTERPRISE API —
